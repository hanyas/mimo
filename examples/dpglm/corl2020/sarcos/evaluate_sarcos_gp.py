...
if __name__ == "__main__":
    ...
    for n in range(nb_seeds):
        ...
        # subsample data set because it is too big for GPU
        idx = np.random.choice(44484, 35000, replace=False)
        _train_input = train_input[idx, :]
        _train_target = train_target[idx, :]

        for i in range(nb_outputs):
            target_transform.fit(target_data[:, i][:, None])

            _gp = GPRegressor(input_size=21, device='gpu',
                              input_transform=input_transform,
                              target_transform=target_transform)

            _gp.fit(_train_target[:, i], _train_input, nb_iter=100, lr=0.05,
    ...
    dt.to_csv('sarcos_gp.csv', mode='a', index=True)

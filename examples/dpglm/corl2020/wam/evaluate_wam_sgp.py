...
if __name__ == "__main__":
    ...
    for n in range(nb_seeds):
        ...
        for i in range(nb_outputs):
            ...
            _gp = SparseGPRegressor(train_input, inducing_size=2500, device='gpu',
                                    input_transform=input_transform,
                                    target_transform=target_transform)

            _gp.fit(train_target[:, i], train_input, nb_iter=500,
                    lr=0.01, preprocess=True, verbose=False)
    ...
    dt.to_csv('wam_sgp.csv', mode='a', index=True)

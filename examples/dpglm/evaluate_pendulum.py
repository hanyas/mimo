...
if __name__ == "__main__":
    ...
    nb_train_rollouts, nb_train_steps = 25, 500
    ...
    _train_input = np.vstack([np.hstack((_x[:-1, :], _u[:-1, :])) for _x, _u in zip(train_obs, train_act)])
    _train_target = np.vstack([_x[1:, :] - _x[:-1, :] for _x in train_obs])

    train_data = {'input': _train_input, 'target': _train_target}
    ...
    _prediction = meanfield_forcast(dpglms[0], test_obs[0][0, :],
                                    ...
                                    exogenous=test_act[0], incremental=True)
    ...
    # _input_scaler = PCA(n_components=dm_obs + dm_act, whiten=True)

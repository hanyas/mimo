...
def matrix_t(data, idx, labels,out_dim, in_dim_niw, affine, x_hat, V_k, M_k, nb_models, S_0, xxT, yxT, yyT, psi_mniw):
    def inv(arg):
    ...
    # print('dot_x_hat_x_hat',dot_x_hat_x_hat)
    # print(dot_xx)
    # print(dot_x_hat_x_hat)
...
def NIW_marg_likelihood(data_x, mu, kappa, psi_niw, nu_niw, N, D):
    ...
    log_term3 = nu_niw / 2 * np.log(np.absolute(psi_niw)) - nu_niw_hat / 2 * np.log(np.absolute(psi_niw_hat)) # Fixme det

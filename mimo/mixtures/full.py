...
class MixtureOfGaussians(Distribution):
    ...
    def rvs(self, size=1):
        ...
        for idx, (g, count) in enumerate(zip(self.components, counts)):
            obs[z == idx, ...] = g.rvs(count)
    ...
    def log_scores(self, obs):
        ...
        for idx, g in enumerate(self.components):
...
class BayesianMixtureOfGaussians(Distribution):
    ...
    @property
    def nb_params(self):
        return self.gating.nb_params + sum(c.nb_params for c in self.components)
    ...
    def max_aposteriori(self, obs):
        ...
        # mixture weights
        self.gating.max_aposteriori(None, [_score for _score in scores])
    ...
    def plot(self, obs=None, color=None, legend=False, alpha=None):
        ...
        self.gating.params = self.gating.posterior.mean()
        for c in self.components:
            c.params = c.posterior.mode()
